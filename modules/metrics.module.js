export function initMetrics(mountEl, opts = {}){
  ...
  function renderHoursUtil(hours){
    const rawAH = hours?.AH;
    const rawTCH = hours?.TCH;
    const rawBH = hours?.BH;

    const AH = Number.isFinite(rawAH) ? Math.max(0, Number(rawAH)) : 0;
    const TCH = Number.isFinite(rawTCH) ? Math.max(0, Number(rawTCH)) : 0;
    const BH = Number.isFinite(rawBH) ? Number(rawBH) : null;
    ...
    elHoursBar.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px; flex-wrap:wrap;">
        <span class="pill">AH: ${fmtMaybe(rawAH)}</span>
        <span class="pill">TCH: ${fmtMaybe(rawTCH)}</span>
        <span class="pill">BH: ${fmtMaybe(rawBH)}</span>
      </div>
