// ✅ FIXED IMPORT PATHS (modules are in same folder)
...
function startApp(){
  ...
  // ------------------------------
  // ✅ LOGO LOADER (tries common paths so no 404 breaks UI)
  // ------------------------------
  function initLogo(){
  ...
  function computeProjectCard(pc, rowsForProject){
    ...
    const hasStageData = (s)=> !!(s.start || s.end || s.extEnd || s.deliverables != null || s.allocated != null || s.consumed != null || s.stagePP != null);
    const effectiveEnd = (s)=>{
      if(s.extEnd instanceof Date && s.end instanceof Date && s.extEnd > s.end) return s.extEnd;
