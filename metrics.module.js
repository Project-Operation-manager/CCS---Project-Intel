export function initMetrics(mountEl, opts = {}){
  ...
  mountEl.innerHTML = `
    <style>
      ...
      .mTitle{ font-size:13px; font-weight:950; color: var(--text); }
      ...
      .mGrid2{ display:grid; grid-template-columns: 180px 1fr; gap:10px; align-items:center; margin-top:10px; }
      @media (max-width: 1200px){ .mGrid2{ grid-template-columns: 1fr; } }
      ...
      .simInput:focus{ border-color: rgba(15,23,42,0.22); box-shadow: 0 0 0 4px rgba(15,23,42,0.06); }

      .simUnit{ font-size:12px; font-weight:900; color: rgba(15,23,42,0.70); text-align:center; }
    </style>
  ...
  function renderHoursUtil(hours){
    ...
    elHoursBar.innerHTML = `
      ...
      <div style="position:relative; height:20px; border-radius:999px; background: rgba(15,23,42,0.06); border:1px solid rgba(15,23,42,0.10); overflow:visible;">
        <div style="position:absolute; left:${basePct}%; top:-6px; bottom:-6px; width:2px; background:rgba(15,23,42,0.18);"></div>

        <div style="position:absolute; left:0; top:0; bottom:0; width:${withinPct}%; border-radius:999px; background: var(--arch);">
          ${insideLabel(withinPct)
            ? `<span style="display:flex; align-items:center; justify-content:center; height:100%; font-size:11px; color:rgba(15,23,42,0.92); padding:0 8px; white-space:nowrap; font-weight:950;">${escapeHtml(fmt(within,1))}</span>`
            : `<span style="position:absolute; left:calc(${withinPct}% + 8px); top:50%; transform:translateY(-50%); font-size:11px; color:rgba(15,23,42,0.82); white-space:nowrap; font-weight:950;">${escapeHtml(fmt(within,1))}</span>`
          }
        </div>

        ${exceed > 0 ? `
          <div style="position:absolute; left:${basePct}%; top:0; bottom:0; width:${exceedPct}%; border-radius:999px;
            background-image: repeating-linear-gradient(45deg, rgba(255,255,255,0.00) 0 6px, rgba(255,255,255,0.45) 6px 9px);
    ...
    elDeployLegend.innerHTML = labels.length
      ? labels.map((name, i)=>`
          <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; padding:6px 0; border-bottom:1px solid rgba(15,23,42,0.08);">
            <div style="font-size:12px; font-weight:950; color:rgba(15,23,42,0.88); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(name)}</div>
            <div style="font-size:12px; font-weight:950; color:rgba(15,23,42,0.72);">${escapeHtml(fmt(data[i],1))}%</div>
          </div>
        `).join("")
      : `<div class="mNote">/</div>`;
